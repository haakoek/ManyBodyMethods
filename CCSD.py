...
def I(i,j,a,b,w):
	return QRPS(a+1,b+1,i+1,j+1,w)

def computeT1Amplitudes(N,L,w,F,t1_old,t2_old):
	
	t1_new = zeros((N, (L/2-N)))
	...
	for i in range(0,N):
		for a in range(N,L/2):
			...
			for k in range(0,N):
				for c in range(N,L/2):
					t1_new[i,a-N] += QRPS(c+1,i+1,k+1,a+1,w)*t1_old[k,c-N]
					t1_new[i,a-N] += F[k,c]*t2_old[i,k,a-N,c-N]
					t1_new[i,a-N] -= F[k,c]*t1_old[i,c-N]*t1_old[k,a-N]

			for k in range(0,N):
				for c in range(N,L/2):
					for d in range(N,L/2):
						t1_new[i,a-N] += 0.5*QRPS(c+1,d+1,k+1,a+1,w)*t2_old[k,i,c-N,d-N]
						t1_new[i,a-N] += QRPS(c+1,d+1,k+1,a+1,w)*t1_old[k,c-N]*t1_old[i,d-N]

			for k in range(0,N):
				for l in range(0,N):
					for c in range(N,L/2):
						t1_new[i,a-N] -= 0.5*QRPS(c+1,i+1,k+1,l+1,w)*t2_old[k,l,c-N,a-N]
						t1_new[i,a-N] -= QRPS(c+1,i+1,k+1,l+1,w)*t1_old[k,c-N]*t1_old[l,a-N]
			
			for k in range(0,N):
				for l in range(0,N):
					for c in range(N,L/2):
						for d in range(N,L/2):							
							t1_new[i,a-N] -= QRPS(c+1,d+1,k+1,l+1,w)*t1_old[k,c-N]*t1_old[i,d-N]*t1_old[l,a-N] 
							t1_new[i,a-N] += QRPS(c+1,d+1,k+1,l+1,w)*t1_old[k,c-N]*t2_old[l,i,d-N,a-N]
							t1_new[i,a-N] -= 0.5*QRPS(c+1,d+1,k+1,l+1,w)*t2_old[k,i,c-N,d-N]*t1_old[l,a-N]
							t1_new[i,a-N] -= 0.5*QRPS(c+1,d+1,k+1,l+1,w)*t2_old[k,l,c-N,a-N]*t1_old[i,d-N]
